...
import { readJsonFile, resolveProjectPath } from './utils.ts';
import {
...
/**
 * Converts an ISO timestamp string to epoch milliseconds.
...
/**
 * Type guard: narrows a message to a token-bearing Gemini model message.
 * Requires type='gemini', a non-empty id, and valid token data with input > 0.
 */
export function isTokenBearingGeminiMessage(msg: unknown): msg is GeminiTokenBearingMessage {
  if (!msg || typeof msg !== 'object') return false;

  const candidate = msg as Partial<GeminiTokenBearingMessage>;
  if (candidate.type !== 'gemini') return false;
  if (typeof candidate.id !== 'string' || candidate.id.length === 0) return false;

  const tokens = candidate.tokens;
  if (!tokens || typeof tokens !== 'object') return false;
  if (!Number.isFinite(tokens.input) || tokens.input <= 0) return false;
  if (!Number.isFinite(tokens.output)) return false;
  if (!Number.isFinite(tokens.total)) return false;

  return true;
}

/**
 * Parses a single session file's messages into SessionUsageData rows.
 * Deduplicates by message id.
 */
export async function parseSessionFileRows(
  record: GeminiConversationRecord,
  mtimeMs: number,
  filePath: string,
): Promise<SessionUsageData[]> {
  const deduped = new Map<string, SessionUsageData>();
  const projectPath = await resolveProjectPath(record.directories, filePath);
...
/**
 * Main parse pipeline: discovers session files, applies caching and dirty-path
 * optimizations, and returns aggregated SessionUsageData rows.
 */
export async function parseSessionsFromProjects(
  ...
  try {
    await fs.access(GEMINI_SESSIONS_PATH);
  } catch {
    ctx.logger.debug('No Gemini sessions directory found');
    return [];
  }
  ...
  if (
    ...
    ctx.logger.debug('Gemini: using cached sessions (within TTL)', { count: sessionCache.lastResult.length });
  ...
  if (needsFullStat) {
    ctx.logger.debug('Gemini: full reconciliation sweep triggered');
  }
  ...
  for (const chatsDirPath of chatsDirs) {
    ...
    for (const entry of entries) {
      ...
      const record = await readJsonFile<GeminiConversationRecord>(filePath);
  ...
  // Prune stale metadata entries for files no longer on disk
  ...
  for (const file of sessionFiles) {
    ...
    const record = await readJsonFile<GeminiConversationRecord>(file.filePath);
    if (!record) continue;

    const usageRows = await parseSessionFileRows(record, file.mtimeMs, file.filePath);
  ...
  ctx.logger.debug('Gemini: parsed sessions', {
